...
function Card({ card }: CardComponentProps) {
  const cardStyle = {
    transform: "scale(1)",
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center w-auto gap-2 p-4 text-center duration-300 bg-purple-600 border-2 rounded-lg shadow-lg hover:delay-100 hover:bg-bgCard border-slate-800 bg-opacity-5 bg-blur-sm backdrop-filter backdrop-blur-sm ${
    ...
    >
      ...
      <p
        className={`text-sm font-normal ${
          isHovered ? "text-indigo-700" : "text-white"
        }`}
      >
        {card.text}
      </p>
